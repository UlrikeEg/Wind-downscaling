...
def build_features(stid, merged, meta):
    ...
    return {
        ...
        "model_mean_ws": float(np.nanmean(merged.hrrr_wspd10_h)),
        "model_std_ws": float(np.nanstd(merged.hrrr_wspd10_h)),
        ...
        "z0": float(np.nanmean(merged.fsr_avg)),
        "terrain_complex": float(r.get("mean_std_vicinity", np.nan)),
    }

def read_merged_nc_and_make_df(station_id, resample_freq = "h"):

    # Read merged data
    fpath = os.path.join(base_dir, f"merged_HRRR_ERA5_obs_{station_id}.nc")
    ...
    # Data QC
    merged['obs_windspeed_3m'] = merged.obs_windspeed_3m.where((merged.obs_windspeed_3m >= 0) & (merged.obs_windspeed_3m <= 70), np.nan)

    # Resample to hourly obs data
    merged['obs_windspeed_3m'] = merged['obs_windspeed_3m'].resample(resample_freq).mean()
...
print("Reading metadata and environmental data...")
...
base_dir = "/kfs2/projects/sfcwinds/merged_station_data_updated/"  # all data in US Southwest as nc files
...
# Cut terrain to NM or US SW region
#NM borders
max_lat = 37.2
min_lat = 31.3
max_lon = -103.0   
...
ds = ds.where( (ds.lon > min_lon) & (ds.lon < max_lon), drop=True)  # Cut to borders
ds = ds.fillna(0)   # NaN values are where no elevation is available (e.g. ocean) - set to zero
mean_elev = ds['elevation_mean'].values
std_elev = ds['elevation_std'].values
lat = ds['lat'].values
...
print("Data reading and preprocessing complete.")
...
#%% Quantile Mapping with Machine Learning to New Stations


# -------------------------------
# Step 1: Build training and test datasets
# -------------------------------
...
pattern = os.path.join(base_dir, "merged_HRRR_ERA5_obs_*.nc")   # US SW all data files
merged_files = glob.glob(pattern)
all_stids = [os.path.basename(f).replace("merged_HRRR_ERA5_obs_", "").replace(".nc", "") for f in merged_files]

all_stids = all_stids[::] # !!! e.g. [::100] is for testing a reduced number of stations, remove later

# Pre-filter stations with insufficient usable data
# !!! eventually use a bad-station list to exclude stations from analysis.
valid_stids = []
invalid_stids = []
for stid in all_stids:
    fpath = os.path.join(base_dir, f"merged_HRRR_ERA5_obs_{stid}.nc")
    try:    
        df_tmp = xr.open_dataset(fpath, engine='netcdf4')[["hrrr_wspd10_h", "obs_windspeed", "era5_wspd10"]]
    except Exception:
        invalid_stids.append(stid)
        continue
    # Count minimum non-NaN values across all variables   
    min_non_nan = {}
    for var in df_tmp.data_vars:
        non_nan_count = df_tmp[var].count().item() if df_tmp[var].count().size == 1 else df_tmp[var].count().min().item()
        min_non_nan[var] = non_nan_count
    n_obs = min(min_non_nan.values())
...
# Should be 1696/1710 for the US Southwest
...
train_model = True  # False to load pre-trained model, True to train new model (takes about an hour when running a job on Kestrel)
...
if train_model == True:
    ...
    for i, station_id in enumerate(training_stids):
        ...
        merged = read_merged_nc_and_make_df(station_id)

        # Extract training data (non-missing obs)
        model_train = merged.hrrr_wspd10_h[~np.isnan(merged.obs_windspeed_3m)]
        obs_train = merged.obs_windspeed_3m[~np.isnan(merged.obs_windspeed_3m)]
    ...
    ml_model = RandomForestRegressor(n_estimators=200, random_state=0)   # tune hyperparameters as needed
    ml_model.fit(X, y)
    ...
    try:
        ...
        fig = plt.figure(figsize=(5, 0.35*len(keep_cols)+1))
        ...
        # Save ML model
        joblib.dump(ml_model, "/kfs2/projects/sfcwinds/scripts/sfcwinds_on_kestrel/quantile_mapping_rf_model_all_data.pkl")

        print("ML model saved to quantile_mapping_rf_model_all_data.pkl")
...
feature_cols = X.columns.tolist()  # use when training the model first
...
for i, stid in enumerate(test_stids):
    ...
    try:
        ...
        # Read and process merged data
        merged_test = read_merged_nc_and_make_df(stid)
        ...
        full_obs_series = merged_test.obs_windspeed_3m.values
        ...
        print(f"Processing failed for station {stid}: {e}")

        test_results[stid] = {
            "quantiles": quantiles,
            "model_q": model_q_test,
            "predicted_obs_q": [np.nan]*len(quantiles),
            "original_model_series": model_series,
            "corrected_series": [np.nan]*len(model_series),
            "metrics": {
                "r_model": np.nan,
                "r_corr": np.nan,
                "rmse_model": np.nan,
...
with open("/kfs2/projects/sfcwinds/scripts/sfcwinds_on_kestrel/test_results.pkl", "wb") as f:
...
print("Plotting summary maps of test station results...")
    ...
    for _, row in test_meta.iterrows():
        ...
        if sid in test_results:
            ...
            if np.isfinite(bm) and np.isfinite(bc):
                ...
                if abs(bc) < abs(bm):
                    improved += 1
                diff_vals.append(abs(bm) - abs(bc))
...
for i, fig_num in enumerate(plt.get_fignums()):
    ...
    fig.savefig(f"/kfs2/projects/sfcwinds/scripts/plots/figure_{i+1}.png", dpi=200)
