...
Runtime behavior:
  ...
  - Final merging per station occurs only on rank 0 after an MPI barrier.
...
def save_station_netcdf(per_station, total_output_path, encoding):
    ...
    # Use CF-compliant encoding for valid_time (not datetime64[ns])
    enc = encoding.copy()
    enc['valid_time'] = {
    ...
    per_station.close()
    print(f"[STATUS] Saved {total_output_path}", flush=True)
...
#%% Read all-station HRR files

if rank == 0:

    # Load the datewise separated files 
    ...
    print(f"[STATUS]  {len(good_files)} of {len(file_part_list)} HRRR files are valid.", flush=True)
    

    # Concatenate all files along time dimension
    print(f"[STATUS]  Reading files...", flush=True)
    concat_data = xr.open_mfdataset(good_files, engine='netcdf4', 
                                    concat_dim ="valid_time",combine='nested', chunks={})
    concat_data = concat_data.sortby("valid_time").drop_duplicates("valid_time") # make sure time stamp is sorted

    print(f"[STATUS] {len(good_files)} HRRR files opened.", flush=True)

    # Chunk along time dimension only for efficient access later - Single contiguous chunk along time (avoid many tiny tasks)
    concat_data = concat_data.chunk({'valid_time': 4 * 24 * 30, 'station': 1})  # monthly-sized time chunks; leave vertical/native chunks

    print(f"[STATUS] HRRR files transformed", flush=True)

    # Initialize Dask client for parallel writing
    #client = Client()  # gives a nanny error
    # client = None  # Do not start Client()
    # dask.config.set(scheduler='threads')
    # tasks = []

    print(f"[STATUS] Saving station data ...", flush=True)

    # Loop over all stations and save seperate files
    for i in range(concat_data.sizes['station']):  #range(50): #   [0]: # 

        if i % 10 == 0:
            print (f"[STATUS] Processing station {i+1} of {concat_data.sizes['station']}", flush=True)
        
        # Select HRRR data for this station
        per_station = concat_data.isel(station=i)

        # encoding
        encoding = {v: {'zlib': True, 'complevel': 1, 'shuffle': True} for v in per_station.data_vars}

        # Save file (use dask for parallel writing)
        total_output_path = os.path.join(save_folder, f"HRRR_{per_station.station.values}.nc")

        # Per-station saving (no dask)
        # Ensure valid_time is datetime64[ns]
        if not np.issubdtype(per_station['valid_time'].dtype, np.datetime64):
            per_station = per_station.assign_coords(valid_time=per_station['valid_time'].astype('datetime64[ns]'))
        # Use CF-compliant encoding for valid_time (not datetime64[ns])
        ...
        # Use dask delayed to save in parallel
        # delayed_save = dask.delayed(save_station_netcdf)(per_station, total_output_path, encoding)
        # tasks.append(delayed_save)
        
        # Make a plot comparing wind speed and direction for one station between HRRR and observations
        if i == -5:

            # Read observations according to station filter - read the actual data
            all_dfs = []
            pattern = os.path.join(base_dir, "*", per_station.station.values.item(), f"*.parquet")
            ...
            # Plot obs vs. HRRR for this station
            fig, axs = plt.subplots(2, 1, figsize=(10, 6), sharex=True)

            # Wind speed
            axs[0].plot(obs.timestamp.values, obs.windspeed.values, ".", color="green", label="Obs "+per_station.station.values.item())
            axs[0].plot(per_station.valid_time.values, per_station.wspd10_h.values, ".", color="blue", label="HRRR")
            axs[0].set_ylabel("Wind Speed (m/s)")
            axs[0].legend()
            axs[0].grid(True)

            # Wind direction
            axs[1].plot(obs.timestamp.values, obs.winddirection.values, ".", color="green", label="Obs "+per_station.station.values.item())
            axs[1].plot(per_station.valid_time.values, per_station.wdir10_h.values, ".", color="blue", label="HRRR")
            ...
            plt.savefig(os.path.join(save_folder, "plots", f"{station_id}.png"))
            plt.close()

    # print ("[STATUS] Dask is writing all station files in parallel ...", flush=True)
    # dask.compute(*tasks)
